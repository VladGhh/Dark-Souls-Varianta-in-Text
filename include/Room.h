...
public:
    Room(string name, string desc, T inside ,Player *player) 
        : name(name), description(desc), inside(inside), player(player), visited(false) {
    };
...
void Room<T>::visit(bool &triggerRespawn) {
    ...
    if constexpr (std::is_convertible_v<T, Enemy*>) {
        if (inside && inside->isAlive()) {
            ...
            while (inside->isAlive() && player->getHp() > 0) {
                ...
                // Verificare input Enemy
                cin >> op;
                if (cin.fail()) {
                    if (cin.eof()) break; // S-a terminat fisierul de input
                    cin.clear();
                    cin.ignore(1000, '\n');
                    continue;
                }
    ...
    //Consumable
        ...
        while (!inputValid) {
            ...
            // Verificam daca citirea a esuat (EOF sau input gresit)
            if (cin.fail()) {
                // Daca am ajuns la finalul fisierului (EOF), iesim fortat
                if (cin.eof()) {
                    // FIX: Am sters "inputValid = true;" de aici pentru ca break-ul ne scoate oricum
                    break; 
                }
                cin.clear(); // Resetam starea de eroare
                cin.ignore(1000, '\n'); // Ignoram restul liniei
                cout << "Input invalid, incearca y sau n.\n";
                continue;
            }
