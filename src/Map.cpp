#include "Map.h"
#include <limits> // Necesar pentru input validation

Map::Map(Player* play) : currentRoom(1), score(0), map({
        {},
        {2},
        {1, 3, 4 , 7},
        {2},
    ...
    // Aici functioneaza perfect cu noua clasa KeyItem care foloseste std::string
...
void Map::move() {
    ...
    int choice;
    // FIX: Validare input pentru a preveni bucla infinita daca se introduce text
    if (!(cin >> choice)) {
        cout << "Te rog introdu un numar valid.\n";
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
        return; // Sau continue, depinde cum e apelat, dar aici e void
    }
